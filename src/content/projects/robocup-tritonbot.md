...
This is the chunk I worked on as TritonBot team lead: the runtime bridge that listens to AI commands from the SSL simulation environment, converts those commands into wheel RPM targets, and sends them to the robot’s embedded system—optionally with real-time analytics to compare “expected vs actual.” My commits in the team repository cover the AI/UDP interface, the simulator connection, the vision filter and global-to-local command processor, the generated Protobuf bindings, and the embedded-systems interface; the tuned runtime below grew out of that skeleton.
